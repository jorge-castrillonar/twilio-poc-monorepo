...
4. Keys are shared between `s3-service` and `auth-service` for token validation
...
1. Generate keys in `s3-service/keys/`
2. Copy to `auth-service/keys/` if needed
